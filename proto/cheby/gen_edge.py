...
class Encore(object):
    ...
    def required_access_mode(self, n):
        ...
        for el in n.children:
            if isinstance(el, (tree.Block, tree.Memory)):
                m.append(self.required_access_mode(el))
            elif isinstance(el, tree.Submap):
                m.append(self.required_access_mode(el.c_submap))
            else:
                m.append(el.access)
        m = list(set(m))
        if len(m) == 1:
    ...
    def process_body(self, b, n, offset, res_name, name_prefix=[], block_prefix=[], name_suffix=[], top=False):
        for el in n.children:
            ...
            if isinstance(el, tree.Reg):
                ...
                if len(el.children) == 1 and isinstance(el.children[0], tree.Reg):
                    ...
                    if get_extension(el, 'expand', True):
                        b2 = EncoreBlock(b.encore, el, el_name, res_name)
                        b.append_block(b2, el_name, el_addr, el.comment)
                        for i in range(0, el.count):
                            self.process_body(b2, el, i*el.c_elsize, res_name, name_suffix=[str(i)])
                    else:
                        b2 = EncoreBlock(b.encore, el, el_name, res_name)
                        self.process_body(b2, el, 0, res_name)

                        for i in range(0, el.count):
                            b.append_block(b2, f"{el_name}_{i}", el_addr + i*el.c_elsize, el.comment)
                ...
                if isinstance(el, tree.Submap):
                    ...
                    if not get_extension(el, 'expand', True):
                        ...
                        access = self.required_access_mode(el)
                        b.append_reg(el, el_name, el_addr, access, word_size, el.c_size // word_size, '', el.comment)
                        continue
                ...
                if include:
                    self.process_body(b, node, el_addr, res_name, el_name_cat if use_block_prefix else name_prefix, prefix)
                else:
                    b2 = EncoreBlock(b.encore, el, name, res_name)
                    b.append_block(b2, el_name, el_addr, el.comment)
                    self.process_body(b2, node, 0, res_name, [], prefix)
    ...
    def write(self, fd):
        ...
        hw_mod_name = get_extension(self.root, 'module-type', self.root.name)
        hw_lif_name = get_extension(self.root, 'board-type', hw_mod_name.lower())
        hw_lif_vers = str(get_extension(self.root, 'driver-version', '1.0.0')) + \
                      str(get_extension(self.root, 'driver-version-suffix', ''))
        edge_vers = str(get_extension(self.root, 'schema-version', '3.0'))
        bus = get_extension(self.root, 'bus-type', 'VME')
        endian = get_extension(self.root, 'endianness', 'big' if bus == 'VME' else 'little')
        description = get_extension(self.root, 'description', self.root.comment)
        ...
        if bus != 'VME':
            ...
            if bus == 'PCI':
                args = clean_args({
                    'subvendor': get_extension(self.root, 'device-info/subvendor-id'),
                    'subdevice': get_extension(self.root, 'device-info/subdevice-id'),
                }, ('subvendor', 'subdevice'), (hex, hex))

            elif bus == 'VME64x':
                args = clean_args({
                    'revision': get_extension(self.root, 'device-info/revision-id', required=True),
                }, ('revision',), (hex,))

            elif bus == 'PLATFORM':
                args = ''

            vendor = hex(get_extension(self.root, 'device-info/vendor-id', required=True))
            device = hex(get_extension(self.root, 'device-info/device-id', required=True))
        ...
        for i, el in enumerate(self.root.children):
            ...
            args = {
                'addrspace': get_extension(el, 'addr-mode', self.vme_addr_space(self.root.bus)),
                'dwidth': get_extension(el, 'data-width', self.root.c_word_size * tree.BYTE_SIZE),
                'size': el.c_size,
            ...
            self.process_body(self.top, el, 0, el.name, top=True)
